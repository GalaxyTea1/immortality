...
function Cultivation() {
  ...
  const { player, foundation, innerDemon, reputation, alchemy } = gameState;
  
  // Nhật ký hoạt động
  const [activityLog, setActivityLog] = useState([
    { time: '10:42 AM', message: 'Bạn đã hấp thụ <span class="text-primary">5 Linh Khí</span> từ thiên địa.', type: 'primary' },
  ]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const expGain = Math.floor(Math.random() * 3) + 1; // 1-3 EXP mỗi giây
      addExp(expGain);
      
      // Hồi phục căn cơ khi thiền định (mỗi 5 giây)
      if (Math.random() < 0.2) {
        recoverFoundation(1);
      }
      
      // Trấn áp tâm ma khi thiền định (mỗi 10 giây)
      ...
      if (Math.random() < 0.1) { // 10% chance
        addLog(`Tâm cảnh ổn định, bạn nhận được <span class="text-success">+${expGain * 2} Linh Lực</span> bonus!`, 'success');
        addExp(expGain * 2);
      }
    }, 1000);

    return () => clearInterval(interval);
  }, [isMeditating, addExp, addLog, recoverFoundation, suppressInnerDemon, innerDemon.value]);
    ...
    <div className="cultivation-page">
        ...
        <aside className="activity-panel">
          ...
          <div className="activity-list">
            {activityLog.map((log, index) => (
              <div key={index} className={`activity-item ${log.type ? `activity-${log.type}` : ''}`}>
        ...
        <main className="cultivation-main">
          <div className="cultivation-status">
            ...
            <p className="realm-progress-text">
              Tiến độ: <span className="text-white">{formatNumber(player.exp)}/{formatNumber(player.maxExp)} Linh Lực</span>
            </p>
          </div>
          ...
          >
            ...
            <div className="cultivator-avatar">
              <div 
                ...
                style={{
                  backgroundImage: 'url("https://lh3.googleusercontent.com/aida-public/AB6AXuCF3Aag1dBQdTNq1JiA8_1szNXqCQt-FzK9Uzs6jZudf92MXbO0Arc0ubj1zHSunAuycptSEtSAHCmRDYkpDdxNagipry2wMkR0far7tHtkVUFCf2jbA2TXrEfK6twBb7Sawvn1459UQP6Ls2nGBgBOB-eBSIyukBxA6DtEKczaXWm1kxTmhqdASp_5ih4__XxSgUMeh8iH9YFrJEvMYSNw1A3c5B3k0l-4qwdmQoW4hBkmDzbST7-nit6zBSBd9m37LfVXnhSy83s")'
                }}
              ></div>
          ...
          <div className="progress-card glass-panel">
            <div className="progress-header">
              <div>
                ...
                <p className="progress-subtitle">EXP: {formatNumber(player.exp)} / {formatNumber(player.maxExp)}</p>
              </div>
            ...
            <div className="progress-labels">
              <span>{currentRealm.name} Tầng {player.level}</span>
        ...
        <aside className="stats-panel">
          <div className="stat-card">
            <div className="stat-header">
              <span className={`material-symbols-outlined stat-icon stat-icon-${foundationStatus.color === 'success' ? 'primary' : foundationStatus.color}`}>fitness_center</span>
          ...
          <div className="stat-card">
            <div className="stat-header">
              <span className={`material-symbols-outlined stat-icon stat-icon-${demonStatus.color === 'success' ? 'primary' : 'danger'}`}>psychology_alt</span>
      ...
      <div className="mobile-activity glass-panel">
        ...
        <div className="mobile-activity-content">
          &gt; {activityLog[0]?.message.replace(/<[^>]*>/g, '') || 'Không có hoạt động'}
        </div>
      </div>
            ...
            <div className="alchemy-info">
              ...
              <div className="alchemy-exp">
                ...
                <div className="alchemy-exp-bar">
                  <div className="alchemy-exp-fill" style={{ width: `${(alchemy.exp / alchemy.maxExp) * 100}%` }}></div>
                </div>
                <span>{alchemy.exp}/{alchemy.maxExp}</span>
              </div>
            </div>

            {alchemyNotification && (
              <div className={`alchemy-notification ${alchemyNotification.success ? 'success' : 'error'}`}>
